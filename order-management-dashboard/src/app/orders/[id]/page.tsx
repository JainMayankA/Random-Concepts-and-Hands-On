...
import { ArrowLeft, Truck, XCircle, CheckCircle, Package } from 'lucide-react'
...
export default function OrderDetailPage() {
  ...
  async function doAction(path: string, body: object) {
    ...
    try {
      ...
      setShipForm(false); setCancF(false)
    } catch (e: any) {
          ...
          <div className="card">
            ...
            {isCancelled ? (
              <div className="flex items-center gap-3 text-red-600 text-sm">
                <XCircle size={18} />
                <span>Order cancelled{order.cancelled_at ? ` — ${format(new Date(order.cancelled_at), 'MMM d, yyyy h:mm a')}` : ''}</span>
              </div>
            ) : (
          ...
          <div className="card">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between text-gray-500">
                <span>Items</span><span className="font-medium text-gray-900">{order.item_count}</span>
              </div>
              <div className="flex justify-between text-gray-500">
                <span>Total</span><span className="font-medium text-gray-900">${order.total_amount.toFixed(2)}</span>
              </div>
        ...
        <div className="space-y-4">
          <div className="card">
            ...
            <div className="space-y-2">
              ...
              {showShipForm && (
                <div className="space-y-2">
                  <input className="input text-xs" placeholder="Tracking number" value={tracking} onChange={e => setTracking(e.target.value)} />
                  ...
                  <button onClick={() => doAction('ship', { tracking_number: tracking, carrier })} disabled={!tracking || actionLoading}
                    className="btn-primary w-full justify-center text-xs py-2">Ship</button>
                </div>
              )}
              ...
              {showCancelForm && (
                <div className="space-y-2">
                  <input className="input text-xs" placeholder="Cancellation reason" value={cancelReason} onChange={e => setCancelR(e.target.value)} />
                  <button onClick={() => doAction('cancel', { reason: cancelReason })} disabled={!cancelReason || actionLoading}
                    className="btn-danger w-full justify-center text-xs py-2">Confirm cancel</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="card text-xs text-gray-500 space-y-2">
            <div className="flex justify-between"><span>Customer</span><span className="font-mono text-gray-700">{order.customer_id.slice(0,8)}…</span></div>
            <div className="flex justify-between"><span>Placed</span><span>{format(new Date(order.placed_at), 'MMM d, yyyy')}</span></div>
            <div className="flex justify-between"><span>Updated</span><span>{format(new Date(order.last_updated), 'MMM d, h:mm a')}</span></div>
          </div>
        </div>
      </div>
    </div>
...
const DEMO_ORDER: Order = {
  order_id: 'ord-0001-demo', customer_id: 'cust-0001-demo',
  status: 'shipped', total_amount: 149.99, item_count: 3,
