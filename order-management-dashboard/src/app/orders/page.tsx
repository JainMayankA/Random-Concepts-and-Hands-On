      ...
      <div className="card mb-5">
        <div className="flex gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <select className="select w-40" value={status} onChange={e => { setStatus(e.target.value); setPage(0) }}>
      ...
      <div className="card p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              filtered.map(o => (
                <tr key={o.order_id} className="border-b border-gray-50 hover:bg-gray-50 cursor-pointer"
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
          ...
          <div className="flex gap-2">
            <button disabled={page === 0} onClick={() => setPage(p => p - 1)} className="btn-secondary text-xs py-1 px-3 disabled:opacity-40">Previous</button>
            <button disabled={orders.length < PAGE_SIZE} onClick={() => setPage(p => p + 1)} className="btn-secondary text-xs py-1 px-3 disabled:opacity-40">Next</button>
          </div>
        </div>
      </div>
    </div>
...
// Demo orders for when backend is offline
const DEMO_ORDERS: Order[] = Array.from({ length: 12 }, (_, i) => ({
  order_id: `ord-${String(i+1).padStart(4,'0')}-demo`,
  customer_id: `cust-${String((i % 4)+1).padStart(4,'0')}-demo`,
  status: (['placed','confirmed','shipped','delivered','cancelled'] as OrderStatus[])[i % 5],
  total_amount: Math.round((20 + i * 17.5) * 100) / 100,
  item_count: (i % 3) + 1,
  placed_at: new Date(Date.now() - i * 86400000).toISOString(),
  last_updated: new Date(Date.now() - i * 43200000).toISOString(),
}))
